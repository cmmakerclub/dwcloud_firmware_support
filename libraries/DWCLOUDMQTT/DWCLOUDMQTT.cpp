 ...
 */
 ...
 /**
  * \author
  *       Developing more MQTT Qos support and fixebug support DW.CLOUD
  *       The old author doesn't support more AT command library
  *       connect@deaware.com
  *       
  *       Ver 1.1 : Add reconnect function fixed bug can't connect when TCP Faild
  */
...
BOOL ESP::mqttConnect(const char* host, U16 port)
{
  ...
  sprintf(_temp, "AT+CIPSTART=\"TCP\",\"%s\",%d", host, port);
  CHECK(!exec(_temp, "OK", 20000), "TCP: Connection failed");
  INFO("TCP: Connected");
  connState = MQTT_CONNECT_SEND;
}

BOOL ESP::mqttReconnect(void) 
{
  //CHECK(!exec("AT+CIPMUX=0", "OK", 1000), "CWJAP failed");
  exec("AT+CIPCLOSE", "OK", 500);
  CHECK(!exec(_temp, "OK", 20000), "TCP: Connection failed");
